...
import { Eye, EyeOff, Film, StarIcon, Videotape } from "lucide-react";
...
export const fullColumns: ColumnDef<Resource, any>[] = [
	columnHelper.accessor("title", {
		...
		cell: (info) => (
			<span className="inline-flex cursor-pointer items-center text-base hover:underline">
				<span>{info.row.original.type === "FILM" ? <Film /> : <Videotape />}</span>
				<span>
					<Link href={`/resources/${info.row.original.id}`} className="ml-2">
						{info.getValue()}
					</Link>
				</span>
			</span>
	...
	columnHelper.accessor("favourite", {
		id: "favourite",
		cell: (info) => (info.getValue() ? <StarIcon /> : ""),
